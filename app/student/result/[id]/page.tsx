            ...
            <div className="mx-auto max-w-5xl space-y-8">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="bg-indigo-600 p-8 text-white text-center">
                        ...
                        <p className="mt-2 text-indigo-100">{result.totalScore} / {result.maxScore} Marks</p>
                    </div>
                    ...
                    <div className="p-8 grid md:grid-cols-2 gap-8">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-2 flex items-center gap-2">
                                <span className="p-1 rounded bg-orange-100 text-orange-600">⚠️</span>
                                Weak Areas
                            </h3>
                            {result.weakAreas && result.weakAreas.length > 0 ? (
                                ...
                                <p className="text-gray-500 italic">None identified. Great job!</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    {result.answers.map((ans: any, i: number) => {
                        ...
                        return (
                            <div key={i} className={`bg-white rounded-xl p-6 shadow-sm border-l-4 ${ans.isCorrect ? 'border-green-500' : 'border-red-500'}`}>
                                <div className="flex justify-between items-start mb-4">
                                    <h3 className="text-lg font-medium text-gray-900">{qText}</h3>
                                    <div className="flex items-center gap-2">
                                        <span className="font-bold text-gray-900">{ans.marksObtained} / {question?.marks || 1}</span>
                                        {ans.isCorrect ? <CheckCircle className="text-green-500 h-5 w-5" /> : <XCircle className="text-red-500 h-5 w-5" />}
                                    </div>
                                </div>

                                <div className="grid md:grid-cols-2 gap-4 text-sm">
                                    <div className="bg-gray-50 p-3 rounded-lg">
                                        <span className="block text-xs uppercase tracking-wide text-gray-400 mb-1">Your Answer</span>
                                        <p className="text-gray-800 font-medium">{ans.answer?.toString() || "Skipped"}</p>
                                    </div>
                                    {!ans.isCorrect && (
                                        <div className="bg-green-50 p-3 rounded-lg">
                                            <span className="block text-xs uppercase tracking-wide text-green-700 mb-1 opacity-70">Correct Answer / Model</span>
                                            <p className="text-green-800 font-medium">{question?.correctAnswer?.toString() || "N/A"}</p>
                                        </div>
                                    )}
                                </div>
