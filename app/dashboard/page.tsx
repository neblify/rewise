...
export default async function Dashboard() {
    ...
    const avgScore = totalAttempts > 0 ? (totalScorePercentage / totalAttempts).toFixed(1) : 0;
    ...
    const testPerformance: Record<string, { total: number, count: number, title: string }> = {};
        ...
        <div className="min-h-screen bg-gray-50/50 p-8">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-gray-500">Overview of your assessments, student engagement, and performance metrics.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <MetricCard
                        ...
                        value={inventoryCount > 0 ? (totalAttempts / inventoryCount).toFixed(1) : 0}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            {trendData.map((d) => {
                                ...
                                return (
                                    <div key={d.date} className="flex flex-col items-center gap-2 flex-1 group">
                                        <div className="relative w-full bg-indigo-50 rounded-t-lg hover:bg-indigo-100 transition-colors flex items-end justify-center" style={{ height: '100%' }}>
                                            <div
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col">
                        ...
                        <p className="text-sm text-gray-500 mb-4">Tests with the lowest average scores, indicating areas where students struggle.</p>

                        <div className="flex-1 overflow-auto pr-1">
                            {difficultyList.length > 0 ? (
                                <div className="space-y-3">
                                    {difficultyList.map((test, i) => (
                                        <div key={i} className="p-3 rounded-lg border border-gray-100 bg-gray-50 hover:bg-gray-100 transition-colors">
                                            <div className="flex justify-between items-start mb-2">
                                                <h4 className="font-medium text-gray-900 text-sm line-clamp-1" title={test.title}>{test.title}</h4>
                                            </div>
                                            <div className="flex items-center justify-between text-xs">
                                                <span className="text-gray-500">{test.attempts} attempts</span>
                                                <span className={`font-bold px-2 py-0.5 rounded-full ${test.avg < 40 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                                                    }`}>
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100">
                            <Link href="/teacher" className="text-sm text-indigo-600 hover:text-indigo-700 font-medium flex items-center justify-center">
                                View All Tests &rarr;
                            </Link>
                        </div>
                    </div>
