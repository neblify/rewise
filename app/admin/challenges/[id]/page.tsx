...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: test ? `${test.title} | Admin Challenge View | ReWise` : 'Admin Challenge View | ReWise',
  };
}

export default async function AdminChallengeDetailPage(props: Props) {
  ...
  let challengeResults: ChallengeEntry[] = allResults.map((r, attemptIndex) => {
                      ...
                      <ul className="mt-2 space-y-1 text-sm">
                        {q.options.map((opt, idx) => {
                          const isCorrect =
                            ...
                            (q.correctAnswer as unknown[]).includes(idx) &&
                            typeof q.correctAnswer[0] === 'number';
                          const isCorrectString =
                            typeof q.correctAnswer === 'string' &&
                            q.correctAnswer === opt;
                          const isCorrectIndexArray =
                            Array.isArray(q.correctAnswer) &&
                            typeof q.correctAnswer[0] === 'string' &&
                            (q.correctAnswer as string[]).includes(opt);
                          const highlight =
                            isCorrect || isCorrectString || isCorrectIndexArray;
