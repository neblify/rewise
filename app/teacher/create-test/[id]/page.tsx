...
export default function CreateOrEditTestPage() {
    ...
    const [state, formAction] = useActionState(isEditMode ? updateTest : createTest, null);
    ...
    const handleAiGenerate = async () => {
        ...
        try {
            const res = await generateQuestionsAI(aiTopic, aiCount, aiType, aiDifficulty, board, grade);
        ...
        <div className="min-h-screen bg-gray-50 p-8 pb-32">
            <div className="mx-auto max-w-5xl space-y-8">
                <div className="flex justify-between items-end">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">{isEditMode ? 'Edit Test' : 'Create New Test'}</h1>
                ...
                <form action={handleSubmit} className="space-y-8">
                    ...
                    <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100 space-y-6">
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                            <div className="col-span-2">
                                ...
                                <input name="title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500" placeholder="e.g. Science Mid-Term" />
                            </div>
                            ...
                            <div>
                                ...
                                <input name="subject" type="text" value={subject} onChange={(e) => setSubject(e.target.value)} required className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500" placeholder="e.g. Mathematics" />
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-gray-700">Visibility</label>
                                <select name="visibility" value={visibility} onChange={(e) => setVisibility(e.target.value)} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500">
                        ...
                        {sections?.map((section, secIndex) => (
                            <div key={section.id} className="rounded-xl bg-white border border-indigo-100 shadow-md overflow-hidden">
                                ...
                                <div className="p-6 space-y-6">
                                    ...
                                    <button type="button" onClick={() => addQuestionExp(secIndex)} className="w-full py-3 border-2 border-dashed border-gray-200 rounded-lg text-gray-400 hover:border-indigo-400 hover:text-indigo-500 transition-colors flex items-center justify-center gap-2 font-medium">
                    ...
                    <div className="flex justify-between items-center pt-8">
                        <button type="button" onClick={addSection} className="flex items-center gap-2 text-indigo-600 hover:text-indigo-800 font-medium">
                            <Layers className="h-4 w-4" /> Add New Section
                        </button>

                        <button type="submit" className="flex items-center gap-2 rounded-lg bg-indigo-600 px-8 py-3 text-white shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:shadow-xl transition-all font-medium">
                            <Save className="h-5 w-5" /> {isEditMode ? 'Update Test' : 'Publish Test'}
                        </button>
                    </div>
                </form>
            </div>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Number of Questions: {aiCount}</label>
                                <input
                                    type="range"
                                    min="1"
                                    max="10"
