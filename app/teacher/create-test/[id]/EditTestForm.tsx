...
function buildInitialSections(initialData: InitialTestData): Section[] {
  if (initialData.sections && initialData.sections.length > 0) {
    return initialData.sections.map((sec: Section) => ({
      ...sec,
      id: generateId(),
      questions:
    ...
    return [
      {
        ...
        questions: initialData.questions.map((q: Question) => ({
          ...q,
          id: generateId(),
        })),
      },
...
export default function EditTestForm({
  ...
  const [board, setBoard] = useState(
    initialData.board ?? BOARD_PLACEHOLDER_VALUE
  );
  ...
  const [timedState, setTimedState] = useState(() =>
    parseTimedFromTest(initialData)
  );

  // Sections State
  const [sections, setSections] = useState<Section[]>(() =>
    buildInitialSections(initialData)
  );
        ...
        <form action={handleSubmit} className="space-y-8">
          ...
          <div className="rounded-xl bg-card p-6 shadow-sm border border-border space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div>
                <label
                  htmlFor="edit-test-visibility"
                  className="block text-sm font-medium text-muted-foreground"
                >
                  Visibility
                </label>
                <select
                  id="edit-test-visibility"
                  name="visibility"
                  value={visibility}
                  onChange={e => {
          ...
          <div className="flex justify-between items-center pt-8">
            ...
            >
              <Save className="h-5 w-5" /> Update Test
            </button>
          </div>
        </form>
      </div>
          ...
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label
                    htmlFor="edit-ai-difficulty"
                    className="block text-sm font-medium text-muted-foreground mb-1"
                  >
                    Difficulty
                  </label>
                  <select
                    id="edit-ai-difficulty"
                    value={aiDifficulty}
                    onChange={e => {
