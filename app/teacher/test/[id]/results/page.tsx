    ...
    return (
        <div className="min-h-screen bg-gray-50 p-8">
            <div className="mx-auto max-w-7xl space-y-8">
                <div>
                    <Link href="/teacher" className="inline-flex items-center text-sm text-gray-500 hover:text-gray-900 mb-4">
                        <ArrowLeft className="h-4 w-4 mr-1" /> Back to Dashboard
                    </Link>
                    <div className="flex justify-between items-start">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">{test.title} - Results</h1>
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <p className="text-3xl font-bold text-indigo-600 mt-2">
                            {results.length > 0
                                ? Math.round(results.reduce((acc: number, curr: any) => acc + (curr.totalScore / curr.maxScore * 100), 0) / results.length)
                                : 0
                            }%
                        </p>
                    </div>
                                ...
                                {results.map((result: any) => {
                                    ...
                                    return (
                                        <tr key={result._id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-500">{formatDate(result.createdAt)}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-900 font-medium">
                                                    {result.totalScore} / {result.maxScore}
                                                    <span className="ml-2 text-gray-500 font-normal">({percentage}%)</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <Link href={`/student/result/${result._id}`} className="text-indigo-600 hover:text-indigo-900">
                                                    View Details
                                                </Link>
                                            </td>
                                        </tr>
