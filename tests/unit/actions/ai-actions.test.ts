...
// Mock pdf-parse
const mockPdfParse = vi.fn().mockResolvedValue({ text: 'Sample PDF Text content...' });
...
// Import the actions
import { generateQuestionsAI, extractQuestionsFromPdf } from '@/app/teacher/create-test/ai-actions';
    ...
    describe('generateQuestionsAI', () => {
        it('should generate questions successfully', async () => {
            const mockAIResponse = {
                questions: [
                    { text: 'Sample Q?', type: 'mcq', options: ['A', 'B', 'C', 'D'], correctAnswer: 'A', marks: 1 }
                ]
            };
            ...
            const result = await generateQuestionsAI('Math', 1, 'mcq', 'easy', 'CBSE', '10');
